...
impl Vec3 {
	...
	pub fn near_zero(&self) -> bool {
		(self.x_dir.abs() < Self::NEAR_ZERO) && (self.y_dir.abs() < Self::NEAR_ZERO) && (self.z_dir.abs() < Self::NEAR_ZERO)
	}
...
pub fn refract(uv: &Vec3, n: &Vec3, eta_i_over_eta_t: &f64) -> Vec3{
	...
	let r_out_perpendicular: Vec3 = vec3_mul(eta_i_over_eta_t, &vec3_add(uv, &vec3_mul(&cos_theta, n)));
	let r_out_parallel: Vec3 = vec3_mul(&(-(1.0 - vec3_dot(&r_out_perpendicular, &r_out_perpendicular)).abs().sqrt()), n);
