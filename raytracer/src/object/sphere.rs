...
use crate::object::aabb::{AxisAlignedBoundingBoxes, surrounding_box};
...
impl hittable::Hittable for Sphere {
	...
	fn bounding_box(&self, _time0: f64, _time1: f64, output_box: &mut AxisAlignedBoundingBoxes) -> bool {
		output_box.set(
			vec3_sub(&self.center, &Vec3::set(self.radius, self.radius, self.radius)),
			vec3_add(&self.center, &Vec3::set(self.radius, self.radius, self.radius)),
		);
		true
	}
}
...
impl MovingSphere {
	pub fn set(center0: &Vec3, center1: &Vec3, time0: f64, time1: f64, radius: f64, material: &Materials) -> Self {
		Self {
			center0: *center0,
			center1: *center1,
			time0, time1, radius, 
...
impl hittable::Hittable for MovingSphere {
	...
	fn bounding_box(&self, time0: f64, time1: f64, output_box: &mut AxisAlignedBoundingBoxes) -> bool {
		let box0 = AxisAlignedBoundingBoxes::new(
			vec3_sub(&self.center(time0), &Vec3::set(self.radius, self.radius, self.radius)),
			vec3_add(&self.center(time0), &Vec3::set(self.radius, self.radius, self.radius)),
		);
		let box1 = AxisAlignedBoundingBoxes::new(
			vec3_sub(&self.center(time1), &Vec3::set(self.radius, self.radius, self.radius)),
			vec3_add(&self.center(time1), &Vec3::set(self.radius, self.radius, self.radius)),
		);
