...
use crate::basic::vec3::{Vec3, vec3_add, random_unit_vector, reflect, 
	generate_unit_vector, vec3_dot, vec3_mul, random_in_unit_sphere, refract};
...
impl Material for Lambertian {
	fn scatter(&self, _r_in: &Ray, rec: &HitRecord, attenuation: &mut Vec3, scattered: &mut Ray) -> bool {
		...
		scattered.set_value(rec.p, scatter_direction);
...
impl Material for Metal {
	fn scatter(&self, r_in: &Ray, rec: &HitRecord, attenuation: &mut Vec3, scattered: &mut Ray) -> bool {
		...
		scattered.copy_ray(&Ray::set(rec.p, vec3_add(&reflected, &vec3_mul(&self.fuzz, &random_in_unit_sphere()))));
		attenuation.copy_vector(&self.albedo);
...
impl Material for Dielectric {
	fn scatter(&self, r_in: &Ray, rec: &HitRecord, attenuation: &mut Vec3, scattered: &mut Ray) -> bool {
		...
		let cos_theta = { if dt < 1.0 { dt } else { 1.0 }};
		...
		scattered.set_value(rec.p, direction);
		true
	}
}
...
impl Material for Materials {
	fn scatter(&self, r_in: &Ray, rec: &HitRecord, attenuation: &mut Vec3, scattered: &mut Ray) -> bool {
		match self {
			Materials::LambertianMaterials(l) => {
				l.scatter(r_in, rec, attenuation, scattered)
			},
			Materials::MetalMaterials (m) => {
				m.scatter(r_in, rec, attenuation, scattered)
			},
			Materials::DielectricMaterials(d) => {
				d.scatter(r_in, rec, attenuation, scattered)
			}
		}
	}
}
