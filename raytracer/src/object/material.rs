...
use crate::basic::vec3::{Vec3, vec3_add, random_unit_vector, reflect, generate_unit_vector, vec3_dot, vec3_mul, random_in_unit_sphere};
...
impl Material for Materials {
	fn scatter(&self, r_in: &Ray, rec: &HitRecord, attenuation: &mut Vec3, scattered: &mut Ray) -> bool {
		match self {
			Materials::LambertianMaterials(m) => {
