...
use super::{AxisAlignedBoundingBoxes, surrounding_box};
...
impl BvhNode {
	pub fn new_from_vector  (&mut self, src_objects: &[Arc<BvhNode>], start: usize, end: usize, _time0: f64, _time1: f64) -> &Self {
		...
		if object_span == 1 {
			...
			self.left = Arc::new((*self).new_from_vector(&objects, start, mid, _time0, _time1).clone());
			self.right = Arc::new((*self).new_from_vector(&objects, mid, end, _time0, _time1).clone());
		}
...
impl Hittable for BvhNode {
	...
	fn hit(&self, r: &Ray, t_min: &f64, t_max: &f64, rec: &mut super::HitRecord) -> bool {
		...
		let t_clone = rec.t;
		let hit_right: bool = (*self.right).hit(r, t_min, if hit_left {&t_clone} else {t_max}, rec);
		hit_left || hit_right
	}
}
